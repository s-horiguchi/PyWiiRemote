...
class WiiRemoteApp(object):
    def __init__(self):
        self.param = {"width":0, "height":0, # load from conf
                      ...
                      "view-axis":[True, True, True, True, True, True, True, True, True],
                      "acc_thresh":0.05, "gyro_thresh":0.01, #dynamic
                      "maxA":0.1, "maxG":0.1, "maxO":math.pi,
                      "sound_names":["on", "off", "swing1", "strike1", "fall"],
                      "pattern":"No Data",
                      "3Drotate":["average", "acc", "gyro"], }
        ...
        # check exec options
        self.option = {"button":False, "average_orient":False, "accel":False, "acc_orient":False, "sound":False, "fall":False, "gyro":False, "gyro_orient":False, "save":False, "3D":"average", "limited":False,}
        c = 1
        while c <= len(sys.argv[1:]):
            if sys.argv[c] in ["--button", "-b"]:
                ...
                c += 1
            elif sys.argv[c] in ["-3D", ]:
                if sys.argv[c+1] in self.param["3Drotate"]:
                    self.option["3D"] = sys.argv[c+1]
                else:
                    print ("Error!\n"+
                           "Invalid argument for '-3D'.\n"+
                ...
                print (
                    "  Usage: %s [options]\n" % sys.argv[0] + 
                    "\t--3D [type]       3D view by acceleration('acc'), gyro('gyro') or the average('average').'average' is default.\n" +
                    "\t--button,-b       Show press or release button.\n" +
                    "\t--average,-av     Show 3axis the average data.\n" +
    ...
    def change3D(self, c):
        cur_slice = [i for i in xrange(3) if self.option["3D"] == self.param["3Drotate"][i]][0]
        next_slice = cur_slice + c
        if next_slice > 2:
            next_slice = 0
        elif next_slice < 0:
            next_slice = 2
        self.option["3D"] = self.param["3Drotate"][next_slice]
        return
    ...
    def calibration(self, reset_gyro_zero=False):
        ...
        self.visual["wiiobj"].visible = False
    ...
    def accel_gyro(self, raccel, rgyro, rfast, time):
        ...
        if not acc or not gyro_rotate:
            ...
            if self.option["sound"] and self.param["saber"]:
                if (4 > abs(acc_average[0]) >= 1.5) or (4 > abs(acc_average[1]) >= 2.5) or (4 > abs(acc_average[2]) >= 2.5):
            ...
            # free fall detection
            if self.option["fall"]:
                if abs(within[0]) * 10 < 1 and abs(within[1]) * 10 < 1 and abs(within[2]) * 10 < 1 and not pygame.mixer.get_busy():
                    print "Fall!"
                    if self.option["sound"]:
                        self.play_sound("fall")
                
        ## calc average routine ##
        if acc_theta[0]+acc_theta[1] <= (math.pi/2) and acc_theta[1]+acc_theta[2] <= (math.pi/2) and acc_theta[2]+acc_theta[0] <= (math.pi/2):
            ...
            rotate = [((acc_theta[i] - self.data["prev_average_orient"][i])*3 + gyro_rotate[i]*7) / 10 for i in xrange(2)]
        elif acc[1] < 2 and acc[1] < 2 and acc[2] < 2:
            #加速度が各軸2G以下のとき ()
            self.param["pattern"] =  "if 1 < acc[i] < 2"
            rotate = [((acc_theta[i] - self.data["prev_average_orient"][i])*2 + gyro_rotate[i]*8) / 10 for i in xrange(2)]
        else:
            #加速度が各軸2G以上のとき ()
    ...
    def gyro(self, data, fast_mode, frametime):
        ...
        if len(cutoff):
            ...
            if len(self.data["gyro_speed"][0]) >= 3:
                ...
                if self.option["3D"] == "gyro": # 3D
                    self.visual["wiiobj"].rotate(angle=-gyro[0], axis=(1,0,0))
                    self.visual["wiiobj"].rotate(angle=-gyro[1], axis=(0,1,0))
                    self.visual["wiiobj"].rotate(angle=gyro[2], axis=(0,0,1))
    ...
    def init_visual(self, only_wiiobj=False):
        ...
        self.visual["wiiobj"] = visual.box(pos=(0, 0, 0), length=2, height=6, width=1, color=visual.color.white, axis=(2,0,0))
    ...
    def main(self):
        ...
        screen.blit(notes, (self.param["sidebar_width"]*5+self.param["width_"]*5.0/2, self.param["sidebar_height"]*2+self.param["height_"]))
        ...
        while not self.event_handling():
            ...
            screen.fill((255, 255, 255), (0, self.param["height"], self.param["width"]*2, self.param["sidebar_height"]*3))
            ...
            if self.option["3D"] == "acc":
                acc_base.fill((255, 0, 255))
                gyro_base.fill((255, 255, 255))
                average_base.fill((255, 255, 255))
            elif self.option["3D"] == "gyro":
                acc_base.fill((255, 255, 255))
                gyro_base.fill((255, 0, 255))
                average_base.fill((255, 255, 255))
            else: # average
                acc_base.fill((255, 255, 255))
                gyro_base.fill((255, 255, 255))
                average_base.fill((255, 0, 255))
            ...
            screen.blit(font_small.render(self.param["pattern"], True, (0,0,0)), (self.param["sidebar_width"], self.param["height"]+self.param["sidebar_height"]))
        else:
